...
# =======================================================
# ZONA DE EJECUCIÓN: MENÚ INTERACTIVO
# =======================================================

print("\n" + "*"*60)
print(" PREPARANDO DATOS DEL SISTEMA (Imprimiendo datos base)... ")
print("*"*60)

# 1. Pre-calculamos todo una vez para que el menú sea rápido
notas_planas = aplanar_notas(lista_de_estudiantes)
frecuencias_notas = contar_frecuencias(notas_planas)

print("\n" + "-"*60)

texto_limpiado = limpiar_texto(texto)
frecuencias_texto = frecuencia_palabras(texto_limpiado)


# 2. Iniciamos el ciclo del menú principal
while True:
    print("\n" + "="*50)
    print("       SISTEMA DE ANÁLISIS MULTIPROPÓSITO")
    print("="*50)
    print("--- MÓDULO DE NOTAS ---")
    print("1. Calcular la Moda de las notas")
    print("2. Generar Histograma de notas")
    print("3. Clasificar notas por tramos")
    print("\n--- MÓDULO DE TEXTO ---")
    print("4. Ver el Top N de palabras más usadas")
    print("5. Calcular diversidad léxica")
    print("6. Ver análisis de Bigramas (pares de palabras)")
    ...
    if opcion == "1":
        encontrar_moda(frecuencias_notas)

    elif opcion == "2":
        generar_histograma(frecuencias_notas)

    elif opcion == "3":
        # Creamos los tramos de ejemplo para que tu función 3C los clasifique
        tramos_ejemplo = {
            "1.0 - 3.9 (Reprobado)": (1.0, 3.9),
            "4.0 - 5.9 (Aprobado)": (4.0, 5.9),
            "6.0 - 7.0 (Destacado)": (6.0, 7.0)
        }
        print("\n📈 CLASIFICACIÓN POR TRAMOS:")
        resultados = clasificar_en_tramos(notas_planas, tramos_ejemplo)
        for tramo, cantidad in resultados.items():
            print(f"- {tramo}: {cantidad} alumno(s)")

    elif opcion == "4":
        try:
            cantidad = int(input("\n¿Cuántas palabras deseas ver en el Top? (Ej: 5): "))
            top_n_palabras(frecuencias_texto, cantidad)
        except ValueError:
            print("\n❌ Valor inválido. Mostrando el Top 10 por defecto:")
            top_n_palabras(frecuencias_texto, 10)

    elif opcion == "5":
        diversidad_lexica(texto_limpiado)

    elif opcion == "6":
        calcular_bigramas(texto_limpiado)

    elif opcion == "0":
        print("\nCerrando el sistema... ¡Éxito con tu tarea!")
        break
